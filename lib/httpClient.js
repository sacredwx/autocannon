...
function Client (opts, requests = null) {
  if (!(this instanceof Client)) {
    return new Client(opts, requests)
  }
  ...
  if (requests) { this.opts.requests = requests }
  ...
  this.parser[HTTPParser.kOnMessageComplete] = () => {
    ...
    if(responseTime>1000)
      this.slowQueries.push({
        ...
        statusCode: this.resData[this.cer].headers.statusCode
      });
    
    if(this.resData[this.cer].bytes > 100000)
      this.heavyQueries.push({
        url: this.currentRequest.url,
        headers: this.currentRequest.headers,
        statusCode: this.resData[this.cer].headers.statusCode
      })
...
Client.prototype._doRequest = function (rpi) {
  if (!this.rate || (this.rate && this.reqsMadeThisSecond++ < this.rate)) {
    ...
    this.currentRequest=this.requestIterator.currentRequest;
