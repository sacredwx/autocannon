...
function requestBuilder (defaults) {
  ...
  return function buildRequest (reqData) {
    ...
    let path = reqData.url ? reqData.url : reqData.path
    // Replace Old IDs by New Ones
    let id=path.match(/\?id=([^&]*)&/)[0];
    if(id) {
      path=path.replace(id,id.replace(/%2F/g,"-").replace(/%2B/g,"_"));
      path=path.replace(id,id.replace(/\//g,"-").replace(/\+/g,"_"));
    }

    id=path.match(/_wrapper_([^\.]*)\.xml/)[0];
    if(id) {
      path=path.replace(id,id.replace(/%2F/g,"-").replace(/%2B/g,"_"));
      path=path.replace(id,id.replace(/\//g,"-").replace(/\+/g,"_"));
    }
