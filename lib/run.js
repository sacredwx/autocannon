...
function _run (opts, cb, tracker) {
  ...
  function tickInterval () {
    ...
    if (stop) {
      ...
      clients.forEach((client) => {
        fs.appendFileSync(`slow.log`,JSON.stringify(client.slowQueries));
        fs.appendFileSync(`heavy.log`,JSON.stringify(client.heavyQueries));
        client.destroy()
      })
      const result = {
        title: opts.title,
  ...
  function initialiseClients (clients) {
    for (let i = 0; i < opts.connections; i++) {
      ...
      const client = new Client(url, opts.tasks ? opts.tasks[i%opts.tasks.length] : null)
    ...
    function onResponse (statusCode, resBytes, responseTime) {
      ...
      statusCodes[statusCode] = (!statusCodes[statusCode]) ? 1 : statusCodes[statusCode] + 1
